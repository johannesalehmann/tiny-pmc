...
pub struct SafetyAlgorithmCollection<APIdx: Index> {
    good_states: APIdx,
}

impl<APIdx: Index> SafetyAlgorithmCollection<APIdx> {
    pub fn new(good_states: APIdx) -> Self {
        Self { good_states }
    }
}

impl<StateIdx: Index, APIdx: Index> NonstochasticGameAlgorithm<StateIdx>
    for SafetyAlgorithmCollection<APIdx>
{
    type APIdx = APIdx;
    type ModelContext = SafetyAlgorithmContext<StateIdx>;
    ...
    >(
        ...
        let bad_states: Vec<StateIdx> = model
            .states()
            .into_iter()
            .filter(|&state| !model.is_atomic_proposition_set(state, self.good_states))
            .collect();
        let mut buffer = attractor::AttractorBuffer::create(model);
        buffer.reset_owner_counts(model, TwoPlayer::Adam);
...
pub struct SafetyAlgorithmContext<StateIdx: Index> {
    bad_states: Vec<StateIdx>,
    buffer: attractor::AttractorBuffer<StateIdx>,
    initial_state: StateIdx,
}

impl<StateIdx: Index> super::ChangeableOwners<StateIdx> for SafetyAlgorithmContext<StateIdx> {
    fn set_owner(&mut self, index: StateIdx, owner: TwoPlayer) {
        match owner {
            TwoPlayer::Eve => self.buffer.reset_avoiding_player(index),
            TwoPlayer::Adam => self.buffer.reset_reaching_player(index),
        }
    }
}
